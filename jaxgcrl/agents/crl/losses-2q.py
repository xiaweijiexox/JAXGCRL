...
def update_actor_and_alpha(config, networks, transitions, training_state, key):
    def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
        ...
        sa_encoder_params_2, g_encoder_params_2 = (
            critic_params["sa_encoder_2"],
            critic_params["g_encoder_2"],
        )
        ...
        sa_repr_2 = networks["sa_encoder"].apply(sa_encoder_params_2, jnp.concatenate([state, action], axis=-1))
        g_repr_2 = networks["g_encoder"].apply(g_encoder_params_2, goal)

        qf1 = energy_fn(config["energy_fn"], sa_repr, g_repr)
        qf2 = energy_fn(config["energy_fn"], sa_repr_2, g_repr_2)
        qf_pi = jnp.minimum(qf1, qf2)  # twin Q: 取 min 防止过估计
...
def update_critic(config, networks, transitions, training_state, key):
    def critic_loss(critic_params, transitions, key):
        ...
        g_input = transitions.observation[:, config["state_size"]:]

        # twin Q: 两套编码器各自计算 logits
        sa_repr = networks["sa_encoder"].apply(sa_encoder_params, sa_input)
        g_repr = networks["g_encoder"].apply(g_encoder_params, g_input)
        logits1 = energy_fn(config["energy_fn"], sa_repr[:, None, :], g_repr[None, :, :])

        sa_repr_2 = networks["sa_encoder"].apply(sa_encoder_params_2, sa_input)
        g_repr_2 = networks["g_encoder"].apply(g_encoder_params_2, g_input)
        logits2 = energy_fn(config["energy_fn"], sa_repr_2[:, None, :], g_repr_2[None, :, :])

        # InfoNCE loss 对两套分别计算再取均值
        critic_loss = (contrastive_loss_fn(config["contrastive_loss_fn"], logits1) +
                       contrastive_loss_fn(config["contrastive_loss_fn"], logits2)) / 2

        # logsumexp regularisation 对两套分别计算再取均值
        logsumexp = (jax.nn.logsumexp(logits1 + 1e-6, axis=1) +
                     jax.nn.logsumexp(logits2 + 1e-6, axis=1)) / 2
