...
class CRL:
    ...
    # Probability of using nearest neighbor's goal to broaden goal distribution
    cross_episode_ratio: float = 0.1
    ...
    def train_fn(
        ...
        key, buffer_key, eval_env_key, env_key, actor_key, sa_key, g_key, sa_key_2, g_key_2 = jax.random.split(key, 9)
        ...
        sa_encoder_params_2 = sa_encoder.init(sa_key_2, np.ones([1, state_size + action_size]))
        g_encoder_params_2 = g_encoder.init(g_key_2, np.ones([1, goal_size]))
        critic_state = TrainState.create(
            apply_fn=None,
            params={
                "sa_encoder": sa_encoder_params,
                "g_encoder": g_encoder_params,
                "sa_encoder_2": sa_encoder_params_2,
                "g_encoder_2": g_encoder_params_2,
            },
        ...
        def update_networks(carry, transitions):
            ...
            key, critic_key, actor_key, swap_key = jax.random.split(key, 4)

            # Cross-episode nearest-neighbor goal augmentation (Markov property).
            # Within this mini-batch, for each transition find the one with the
            # nearest state (excluding itself). If two states are close, by Markov
            # property their future reachability is similar, so the neighbor's
            # HER goal is a valid alternative goal for the current transition.
            all_states = transitions.extras["state"]  # (B, state_size)
            B = all_states.shape[0]
            state_dists = jnp.sum(
                (all_states[:, None, :] - all_states[None, :, :]) ** 2, axis=-1
            )  # (B, B)
            state_dists = state_dists + jnp.eye(B) * 1e18  # exclude self
            nn_idx = jnp.argmin(state_dists, axis=-1)  # (B,)

            # Neighbor's goal and future
            nn_goal = transitions.observation[nn_idx, state_size:]
            nn_future_state = transitions.extras["future_state"][nn_idx]
            nn_future_action = transitions.extras["future_action"][nn_idx]

            do_swap = jax.random.bernoulli(swap_key, self.cross_episode_ratio, shape=(B, 1))

            current_state = transitions.observation[:, :state_size]
            current_goal = transitions.observation[:, state_size:]
            new_goal = jnp.where(do_swap, nn_goal, current_goal)
            new_obs = jnp.concatenate([current_state, new_goal], axis=1)

            new_future_state = jnp.where(do_swap, nn_future_state, transitions.extras["future_state"])
            new_future_action = jnp.where(do_swap, nn_future_action, transitions.extras["future_action"])

            new_extras = {
                **transitions.extras,
                "future_state": new_future_state,
                "future_action": new_future_action,
            }
            transitions = transitions._replace(observation=new_obs, extras=new_extras)
